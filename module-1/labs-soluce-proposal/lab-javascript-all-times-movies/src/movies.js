...
function bestYearAvg(movies) {
  ...
  return `The best year was ${sorted[0].year} with an average rate of ${
    sorted[0].avg
  }`;
}
...
function bestYearAvg2(movies) {
  ...
  movies.forEach(movie => list[movie.year].push(movie)); // pushing each movie's rate in matching year key
  ...
  for (let year in list) {
    if (list.hasOwnProperty(year)) {
      ...
      if (!best.avg || best.avg < yearAVG) {  // if valid, update best
        best.avg = yearAVG;
        best.year = year;
      }
    }
